...
static int minfs_readdir(struct file *filp, struct dir_context *ctx)
{
	...
	mii = container_of(inode,struct minfs_inode_info, vfs_inode);
	if (!(bh = sb_bread(sb,mii->data_block)))
	{
 ...
 */
/*整体逻辑要从minfs_lookup这个函数说起，这个函数的任务是在一个目录文件中查找一个文件，调用者需要传递一个只初始化了文件名的目录对象dentry，
函数如果找到了对应的文件，就会初始化完成这个dentry，并返回这个dentry的地址，同时会将这个dentry缓存(文件访问的时空局部性)。
